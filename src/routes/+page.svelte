...
<div class="wrapper">
  ...
  <div class="block">
    <p>
      Split tests (or A/B tests) allow you to display different features or variants to test their
      effectiveness. Unfortunately popular options are very pricey, bloated, and don't work SSR.
      This package attempts to remedy all of that.
    </p>
    <p>This package works with <strong>Svelte</strong> and <strong>SvelteKit</strong>.</p>
    <p>
      <em
        >Uses Google Tag Manager by default to keep track of your analytics, but this can be
        replaced with other methods.</em>
    </p>
    ...
    <div class="buttons">
      <a class="button" target="_blank" href="https://github.com/Playpilot/svelte-split-testing"
        >GitHub</a>
      <a class="button" target="_blank" href="https://www.npmjs.com/package/svelte-split-testing"
        >NPM</a>
    </div>
  </div>
  ...
  <div class="block">
    ...
    <p>
      What you are doing with your split tests is completely up to you of course. These examples
    ...
    <p>
      ...
      by adding <code>?force-split-test=[Variant name]</code> to the url. This is only meant for debugging.
    </p>

    <div class="well">
      <div class="buttons">
        <a class="button" data-sveltekit-noscroll href="/">Don't force any test</a>
        <a class="button" data-sveltekit-noscroll href="/?force-split-test=Variant A"
          >Force test A</a>
        <a class="button" data-sveltekit-noscroll href="/?force-split-test=Variant B"
          >Force test B</a>
      </div>
    ...
    <p>
      <small
        >Note: Forcing the variant doesn't work SSR, if you're viewing this page without Javascript
        you might not see a difference. Regular split tests do work during SSR.</small>
    </p>
      ...
      <div class="buttons">
        ...
        {#each Object.entries(colors) as [name, value]}
          <a
            ...
            href="/?force-split-test={name}"
            style:background-color={value}>{name}</a>
        {/each}
      </div>
  ...
  <div class="block">
    ...
    <p>
      When using SvelteKit we need to make sure the same identifier is set on the server as is set
      on the client.
    </p>
    <p>
      In your (main) <code>+layout.server.js</code> load function import and set the identifier and pass
      it along.
    </p>

    <code class="well"
      ><pre>
import &#123; <mark>serverGetSplitTestIdentifier</mark> &#125; from <mark class="string"
          >'svelte-split-testing/splitTesting'</mark>

<mark class="comment">/** @type &#123;import('./$types').LayoutServerLoad&#125; */</mark>
export async function load(&#123; <mark>cookies</mark> &#125;) &#123;
  const <mark>splitTestIdentifier</mark> = <mark>serverGetSplitTestIdentifier</mark>(<mark
          >cookies</mark
        >)
...
&#125;
    ...
    <p>
      Next, in your (main) <code>+layout.js</code> load function pass the identifier along again. We
...
&#125;
    ...
    <p>
      ...
      If you don't, you could simply pass it as <code>return &#123; ...data &#125;</code>.
    </p>
    <p>
      Next, in your (main) <code>+layout.svelte</code> set the identifier using the Context API. Make
    ...
    <code class="well"
      ><pre>
&lt;script&gt;
  import &#123; <mark>clientGetSplitTestIdentifier</mark> &#125; from <mark class="string"
          >'svelte-split-testing/splitTesting'</mark>
  import &#123; <mark>setContext</mark> &#125; from <mark class="string">'svelte'</mark>

  export let data

  <mark>setContext</mark>(<mark class="string">'splitTestIdentifier'</mark>, data?.<mark
          >splitTestIdentifier</mark
        >)
&lt;/script&gt;
...
&lt;slot /&gt;
  ...
  <div class="block">
    ...
    <p>
      For Svelte (without Kit) we do not yet need any set up. There might be some set up depending
      on your needs, but we will get to that in the Usage section.
    </p>
  </div>
  ...
  <div class="block">
    ...
    <code class="well">
      import &#123; <mark>SplitTest</mark> &#125; from
      <mark class="string">'svelte-split-testing'</mark><br />
      <br />
      &lt;<mark>SplitTest</mark>&gt;...&lt;/<mark>SplitTest</mark>&gt;
    </code>

    <p>
      At it's most basic <code>SplitTest</code> is a wrapper component that takes care of some
      business, but for you as the user it's really just a slot. This components has a slot prop
      called <code>variant</code>, this will be used by you to determine what is shown in what
      variant.
    </p>
    <p>
      When using the component there's 2 important props: <code>key</code> and
      <code>variants</code>.
    </p>
    <p>
      <code>key</code> Is the name of the split test. It isn't very important for functionally what this
      key is, is doesn't even need to be unique. It's only used to identify what split test is for your
      analytics tracking.
    </p>
    <p>
      <code>variants</code> Is an array of strings with each variant you want to include. What the
      names are is not important, but this name is what will be used in your analytics tracking. You
      could go for something simple like <code>['A', 'B']</code>, or give them more explicit names
      like <code>['Large Sign Up', 'Small Sign Up']</code>, it's up to you.
    </p>
  </div>
  ...
  <div class="block">
    ...
    <p>
      ...
      prop <code>variant</code> to determine what to show for each variant.
    </p>
...
&lt;<mark>SplitTest</mark>
  ...
  <mark>variants</mark>=&#123;[<mark class="string">'Variant A'</mark>, <mark class="string"
          >'Variant B'</mark
        >]&#125;
...
&lt;/<mark>SplitTest</mark>&gt;
    ...
    <p>
      You don't have to use <code>if</code> statements, you could use it like any other variable.
    </p>
...
&lt;/<mark>SplitTest</mark>&gt;
  ...
  <div class="block">
    ...
    <p>
      By default the Split Test will use Google Tag Manager, using the data layer, to keep track of
      your events. This can be changed to any other method, which will we go in to later.
    </p>

    <p>
      When a split test is shown an event is send to GTM with the given key as the label, and the
      current variant as the value.
    </p>

    <p>
      To fire an event when a button is clicked, or any other action is performed, you can use the <code
        >performAction</code> slot prop. This will fire an event to GTM. You can optionally pass a value
      to change the event type from 'click' to whatever else.
    </p>

    <p>
      The data sent looks a little like: <code
        >&#123; event: 'Split Test', action: [given action], label: [given key], value: [current
        variant] &#125;</code
      >.
    </p>
...
&lt;/<mark>SplitTest</mark>&gt;
    ...
    <p>
      ...
      the <code>performAction</code> function. Once again, this is just for tracking purposes, what the
...
&lt;<mark>SplitTest</mark>
  ...
  &lt;form on:submit=&#123;() =&gt; <mark>performAction</mark>(<mark class="string"
          >'form submit'</mark
        >)&#125;&gt;
    ...
  &lt;/form&gt;
...
&lt;/<mark>SplitTest</mark>&gt;
  ...
  <div class="block">
    ...
    <p>
      If you do not wish to use GTM you can bring your own solution. Simply pass a function to the <code
        >onView</code> property, and this function will be used instead. This function will be fired
      every time the component is mounted in CSR. It will not fire during SSR.
    </p>

    <code class="well"
      ><pre>
&lt;<mark>SplitTest</mark> <mark>onView</mark
        >=&#123;(data) =&gt; console.log(data)&#125;&gt;...&lt;/<mark>SplitTest</mark>&gt;
</pre></code>
    <code class="well"
      ><pre>
&lt;script&gt;
...
&lt;<mark>SplitTest</mark> <mark>onView</mark>=&#123;customTracking&#125;&gt;...&lt;/<mark
          >SplitTest</mark
        >&gt;
</pre></code>

    <p>
      The data returns an object <code>&#123; key, variant, action: 'view' &#125;</code>. What you
...
&lt;<mark>SplitTest</mark> <mark>onView</mark>=&#123;customTracking&#125;&gt;...&lt;/<mark
    ...
    <p>
      To track custom events other than views, you can use whatever method you like and use the <code
        >variant</code> slot prop to determine the current variant.
    </p>
...
&lt;/<mark>SplitTest</mark>&gt;
  ...
  <div class="block">
    ...
    <p>
      This function is used in the set up to set the identifier for SSR. The second parameter in
      this function is an object of options.
    </p>
    <p>
      <code class="well">
    ...
    <p>
      <code>userIdentifier</code> is used to pass an identifier to the function that will be used instead
    ...
    <p>
      <code>cookieName</code> can be used to change the name of the cookie. Defaults to
      <code>splitTestIdentifier</code>
    </p>
    <p>
      If you are setting this as done in the set up, you will need to make sure to pass it along to
      the client side as well, using <code>clientGetSplitTestIdentifier</code> with the same options,
    ...
    <code class="well"
      >setContext('splitTestIdentifier', data?.splitTestIdentifier || <mark
        >clientGetSplitTestIdentifier</mark
      >(&#123; userIdentifier, cookieName &#125;))</code>

    <h4>clientGetSplitTestIdentifier</h4>
    <p>
      This used to set the identifier client side, this is optional if you are also using SSR. If
      you are using Svelte without Kit, this will be the only function you use. The first and only
      parameter is an object of options.
    </p>
    <p>
      <code class="well">
...
if (variant === <mark class="string">"A"</mark>) doThingA()
    ...
    <p>Additionally the slot prop <code>variant</code> can be bound to a variant, allowing it to be re-used for things outside of the component.</p>
...
&lt;<mark>SplitTest</mark> bind:<mark>variant</mark> /&gt;
  ...
  <div class="block">
    ...
    <div class="table">
      ...
      <code>key</code> <code>'Some Key'</code>
      <div>
        Key used to identify the current Split Test. This is primary used during analytics tracking.
      </div>
      <code>variants</code> <code>['Variant A', 'Variant B']</code>
      <div>
        An array of variant names. Can be as many variants as you like. What the names are is not
        important, but they show up during analytics tracking.
      </div>
      <code>onView</code> <code>null</code>
      <div>
        Optional function to be passed to track views of the current variant. Replaces the default
        GTM method.
      </div>
    </div>
    ...
    <div class="table">
      ...
      <code>serverCookies</code> <code>null</code>
      <div>Cookies object as served from +layout.server.js during SSR.</div>
      <code>options</code> <code>&#123; userIdentifier, cookieName &#125;</code>
      <div>Object of configurable options</div>
      <code>options.userIdentifier</code> <code>null</code>
      <div>
        An optional user identifier to use as the identifier. This is used to show a user the same
        split test across different devices, as long as they have the same identifier. If an
        identifier was already set before the user identifier was given the original cookie will be
        used instead. Be aware that this value will be saved in the cookie as a plain string. Do not
        use any data that you might not want to be public.
      </div>
